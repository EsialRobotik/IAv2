package esialrobotik.ia.actions.ax12;
...
public class ActionPool {
	...
	public void doActions(int retryPerAction, long timeout) throws ActionPoolException {
		...
		resetActions();
		// Trigger esialrobotik.ia.actions
		...
		do {
			...
			for (ActionWrapper aw : actionList) {
				...
				try {
					...
					Thread.sleep(5); // Dont overflow esialrobotik.ia.actions
				} catch (ActionException | InterruptedException e) {
					e.printStackTrace();
					aw.retryCount++;
					if (aw.retryCount > retryPerAction) {
						throw new ActionPoolException("An esialrobotik.ia.actions exceeded the max retry limit : " + e.getMessage(), e);
					}
					doAnotherLoop = true;
				}
			}	
		...
		// Wait end of esialrobotik.ia.actions
		do {
			doAnotherLoop = false;
			for (ActionWrapper aw : actionList) {
				if (System.currentTimeMillis() > timeout) {
	...
	/**
	 * Keep state info about esialrobotik.ia.actions
	 */
