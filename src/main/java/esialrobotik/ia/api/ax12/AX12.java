...
public class AX12 {
	...
	private static final int AX12_ADDRESS_MIN = 0;
	private static final int AX12_ADDRESS_MAX = 253;
	static final int AX12_ADDRESS_BROADCAST = 254;
	...
	// Les erreurs possibles retournées par un AX12
	 ...
	 * @param speed
	 * @param alink
	 * @throws IllegalArgumentException if esialrobotik.ia.actions address is not valid
	 */
	 ...
	 * @param ax12 l'esialrobotik.ia.actions à reset
	...
	protected static String validatePacket(byte[] packet, int ax12Addr) {
		...
		if (AX12.unsignedByteToInt(packet[2]) != ax12Addr) {
			return "Le paquet ne contient pas le bon id de l'esialrobotik.ia.actions";
		}
