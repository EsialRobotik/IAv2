...
public class FenwickFouilleGauche4 extends ActionReflexiveAbstract {
    ...
    public void execute() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                executeSubActions(ActionFileBinder.ActionFile.A2022_FENWICK_SONDE_GAUCHE_OUT.ordinal());
                ...
                executeSubActions(ActionFileBinder.ActionFile.A2022_FENWICK_SONDE_GAUCHE_IN.ordinal());
                if (probeResult.equals("fouille1OK")) {
                    executeSubActions(ActionFileBinder.ActionFile.A2022_FENWICK_BRAS_GAUCHE_OUT.ordinal());
                    executeSubActions(ActionFileBinder.ActionFile.A2022_FENWICK_BRAS_GAUCHE_IN.ordinal());
                }
                finished = true;
            }
        }).start();
